...
function ConfigureInterviewPage() {
  ...
  const available = QUESTIONS.filter((q) => topics.includes(q.topic)).length;
  const counts = [3, 5, 8].filter((count) => count <= available);
  const safeCount = counts.includes(questionCount) ? questionCount : counts[counts.length - 1] ?? available;
  return <main className="app-shell"><section className="panel config-panel"><p className="eyebrow">INTERVIEW CONFIGURATION</p><h2>Set your interview level</h2><p className="muted">Configure the session before BerryBot starts asking questions.</p><div className="config-grid"><div className="config-card"><h3>Topics</h3><div className="chip-list">{topics.map((topic) => <span className="chip" key={topic}>{topic}</span>)}</div></div><div className="config-card"><h3>Difficulty</h3><div className="option-row">{(['Easy', 'Medium', 'Hard'] as Difficulty[]).map((level) => <button key={level} type="button" className={`option-button${difficulty === level ? ' active' : ''}`} onClick={() => setDifficulty(level)}>{level}</button>)}</div></div><div className="config-card"><h3>Questions <small>({available} available)</small></h3><div className="option-row">{counts.length ? counts.map((count) => <button key={count} type="button" className={`option-button${safeCount === count ? ' active' : ''}`} onClick={() => setQuestionCount(count)}>{count}</button>) : <span className="muted">No questions available</span>}</div></div></div><div className="selection-actions"><button className="secondary-button" type="button" onClick={() => navigate('/interviews/new')}>Back</button><button className="primary-button" type="button" disabled={!safeCount} onClick={() => navigate('/interviews/session', { state: { topics, difficulty, questionCount: safeCount } })}>Start interview</button></div></section></main>;
}
